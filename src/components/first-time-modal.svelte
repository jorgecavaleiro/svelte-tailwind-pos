<script lang="ts">
  ...
  function requestData(): any {
    dispatch("startWithSampleData");
  }

  function resetFirstTime(): any {
    dispatch("startBlank");
  }
</script>
...
>
  <div class="w-96 rounded-2xl p-8 bg-white shadow-xl">
    ...
    <div class="text-left">
      <button
        ...
        class="text-left w-full mb-3 rounded-xl bg-gray-500 text-white focus:outline-none hover:bg-cyan-400 px-4 py-4"
      >
        <svg
      ...
      <button
        on:click={resetFirstTime()}
        class="text-left w-full rounded-xl bg-gray-500 text-white focus:outline-none hover:bg-teal-400 px-4 py-4"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          class="h-6 w-6 inline-block -mt-1 mr-2"
